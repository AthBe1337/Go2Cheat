...
NTSTATUS DriverMain(PDRIVER_OBJECT driver_object, PUNICODE_STRING registry_path) {
	...
	RtlInitUnicodeString(&device_name, L"\\Device\\G2CDriver");
	...
	RtlInitUnicodeString(&symbolic_link, L"\\DosDevices\\G2CDriver");
	...
	debug_print("[+] Driver successfully initialized.\n");

	return status;
}

NTSTATUS DriverEntry() {
	debug_print("[+] Hello Driver!\n");

	UNICODE_STRING driver_name = {};
	RtlInitUnicodeString(&driver_name, L"\\Driver\\G2CDriver");
